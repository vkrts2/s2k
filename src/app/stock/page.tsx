...
export default function StockPage() {
  ...
  const handleFormSubmit = useCallback(async (data: Omit<StockItem, 'id' | 'createdAt' | 'updatedAt'> | StockItem) => {
    ...
    try {
      ...
      toast({
        title: editingStockItem ? "Stok Kalemi Güncellendi" : "Stok Kalemi Eklendi",
        description: `${savedItem.name} adlı stok kalemi başarıyla ${editingStockItem ? 'güncellendi' : 'kaydedildi'}.`,
      });
      loadItems();
    } catch (error) {
      console.error("Stok kalemi kaydedilirken hata oluştu:", error);
      toast({
        title: "Hata",
        description: "Stok kalemi kaydedilirken bir sorun oluştu.",
        variant: "destructive",
      });
    }
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Stok Kalemi Silindi",
        description: `"${itemToDelete?.name || 'Seçili'}" adlı stok kalemi başarıyla silindi.`,
      });
      setDeletingStockItemId(null);
      loadItems();
    } catch (error) {
      console.error("Stok kalemi silinirken hata oluştu:", error);
      toast({
        title: "Hata",
        description: "Stok kalemi silinirken bir sorun oluştu.",
        variant: "destructive",
      });
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <Button onClick={openAddModal}>
          <PlusCircle className="mr-2 h-4 w-4" /> Yeni Stok Kalemi Ekle
        </Button>
      </div>
      ...
      <Dialog open={showFormModal} onOpenChange={(isOpen) => {
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingStockItem ? "Stok Kalemini Düzenle" : "Yeni Stok Kalemi Ekle"}</DialogTitle>
            <DialogDescription>
              {editingStockItem ? "Stok kalemi bilgilerini güncelleyin." : "Yeni stok kalemi için bilgileri girin."}
            </DialogDescription>
          </DialogHeader>
          <StockForm
      ...
      <AlertDialog open={!!deletingStockItemId} onOpenChange={(isOpen) => { if(!isOpen) setDeletingStockItemId(null);}}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bu işlem geri alınamaz. Bu stok kalemi kalıcı olarak silinecektir.
              Bu kaleme bağlı alım veya satış kayıtları varsa, bu bağlantılar etkilenebilir (ileride eklenecek bir özellik).
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
