...
import { collection, query, where, getDocs, addDoc, updateDoc, deleteDoc, doc, orderBy, setDoc } from "firebase/firestore";
import { getDoc, limit } from "firebase/firestore";
import { getStorage, ref, uploadBytes, getDownloadURL } from "firebase/storage";
import { formatISO, parseISO, format, addDays } from 'date-fns';
import type { Customer, Sale, Payment, Currency, Supplier, Purchase, PaymentToSupplier, TodoItem, PortfolioItem, ArchivedFile, UsefulLink, StockItem, Price, Quotation, QuotationItem, ContactHistoryItem, SupplierTask, Cost } from "./types";
...
export const getCosts = async (userId: string): Promise<Cost[]> => {
  try {
    ...
    const querySnapshot = await getDocs(q);
    return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Cost));
  } catch (error) {
