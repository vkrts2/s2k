...
export const getCustomers = async (uid: string): Promise<Customer[]> => {
  ...
  try {
    // Hem customers koleksiyonundan hem de portfolioItems koleksiyonundan müşteri verilerini çek
    ...
    // Portfolio items'dan müşteri verilerini çek
    const portfolioQuery = query(_getUserCollectionRef(uid, "portfolioItems"), orderBy("createdAt", "desc"));
    const portfolioSnapshot = await getDocs(portfolioQuery);
    const portfolioCustomers = portfolioSnapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => {
      const data = doc.data();
      return {
        id: `portfolio_${doc.id}`,
        name: data.companyName || '',
        email: data.email || '',
        phone: data.phone || data.gsm || '',
    ...
    // Her iki kaynaktan gelen müşterileri birleştir ve tekrarları kaldır
    const allCustomers = [...customers, ...portfolioCustomers];
