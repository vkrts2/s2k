...
export function SupplierDetailPageClient({ supplier: initialSupplier, initialPurchases, initialPaymentsToSupplier }: SupplierDetailPageClientProps) {
  ...
  const [balances, setBalances] = useState<Record<Currency, number>>({ USD: 0, TRY: 0 });
  ...
  const handlePurchaseFormSubmit = useCallback(() => {
    ...
    if (editingPurchase) {
      ...
      const updatedPurchase = storageUpdatePurchase(updatedPurchaseData);
      ...
      const newPurchase = addPurchase(newPurchaseData);
